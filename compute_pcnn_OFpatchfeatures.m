function compute_pcnn_OFpatchfeatures(video_names,param,SplitT)
...
for vi = 1:length(video_names)        % parfor/for
    ...
    featdir_app = sprintf('%s/%s',featdir_app,vidname);
    featdir_flow = sprintf('%s/%s',featdir_flow,vidname);
    ...
    Xn_train = cat(1,Xn_train,Xn_trainOF); 
    clear Xn_trainOF;
...
%     if param.compute_kernel
        ...
        kernelsavename=sprintf('%s/%sKtest.mat',param.savedir,vidname);
        disp(['Save test kernel in: ',kernelsavename])
        assert(sum(isinf(Ktest(:)))==0 && sum(isnan(Ktest(:)))==0)
        save(kernelsavename,'Ktest','-v7.3')
%         clear Ktest;
%         clear Xn_test;
        ...
        kernelsavenamesavename=sprintf('%s/%sKtrain.mat',param.savedir,vidname);
        disp(['Save train kernel in: ',kernelsavenamesavename])
        save(kernelsavenamesavename,'Ktrain','-v7.3')
%         clear Ktrain;
%         clear Xn_train;

%     else
        savename=sprintf('%s/%sXn_test.mat',param.savedir,vidname);
        disp(['Save test features in: ',savename])
        save(savename,'Xn_test','-v7.3')
        savename=sprintf('%s/%sXn_train.mat',param.savedir,vidname);
...
%     param.trainsplitpath = 'JHMDB/splits';
%     param.testsplitpath = 'JHMDB/splits';
