function extract_cnn_OFpatches2(video_names,param)
...
for vi = 1:length(video_names)        % parfor/for
    ...
    for svi = 1:subnb_vid
        ...
        if exist(sprintf('%s/patches_app/image_patch1/%s/%s_im%05d.jpg',param.cachepath,vidname,subvidname,1),'file')
            continue;
        end
    ...
    %         impath = sprintf('%s/%s/%s',imdirpath,vidname,images{fileIndex});
        ...
        for i = 1:2 % appearance and flow
            ...
            for idim = 1:numImages        % min(numImages,length(positions))
...
%                 imwrite(fullbody,sprintf('%s/patches_%s/full_body/%s/%s_im%05d.jpg',param.cachepath,suf{i},vidname,subvidname,idim));
        ...
        if methodchoose==21             
            ...
            [MaskBlock1Pixle2,E_blockmask,PixelMask,E_blockmaskSize] = TEmatrixFindMasksChangingblocksize(E_hat, 0.1, 4, h, w);
            PixelEasyMask = TEasyPixelMask(E_hat, 0.11, h, w);
    ...
    %         [MaskBlock1Pixle2 E_blockmask] = gzEmatrixFindMasksChangingblocksize(RelativeChange, 0.2, 4);
            ...
            TargetCoord = gzGetTarget(PixelEasyMask,PixelSeedMask,uflowdata,vflowdata,magflowdata,h,w);   % gzGetTarget0;gzGetTarget

%             % Save the Patches Coordinates
%             SeqsPatches{vi} = TargetCoord;
            
            % Extract Patches
            positions = [];
            TargetPatchImg0(data,TargetCoord,param,vidname,subvidname,netApp,netFlow,length(positions),uflowdata,vflowdata,magflowdata,h,w);
